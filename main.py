...
###### Header #####
background = Image.open('header.jpg')
st.image(background)

row0_spacer1, row0_1, row0_spacer2, row0_2, row0_spacer3 = st.columns((.1, 2.3, .1, 1.3, .1))
with row0_1:
    #st.title('Key Performance Indicator (KIP) DQ-Store')
    st.subheader('Streamlit App by [Bachtiyar M. Arief](https://id.linkedin.com/in/bachtiyarma)')
with row0_2:
    st.text(" ")
    
row1_spacer1, row1_1, row1_spacer2 = st.columns((.1, 3.2, .1))
with row1_1:
    st.markdown('**Key Performance Indicator (KPI)** adalah alat ukur kuantitatif yang menggambarkan efektivitas perusahaan. Tidak hanya soal seberapa besarnya angka, namun juga apakah performa perusahaan sudah sesuai dengan tujuan bisnis yang ditetapkan sebelumnya.\
                 **DQ-Store** sebuah perusahaan yang bergerak dibidang retail (bisnis yang melibatkan penjualan barang atau jasa kepada konsumen dalam jumlah satuan/eceran) menetapkan tiga kriteria KPI-nya yakni *Total Revenue*, *New Customer Growth* dan *Best Seller Product*\
                 (dalam jangka waktu bulan). Dengan mengukur ketiga KPI tersebut diharapkan perusahaan mampu mengevaluasi performa bisnis dan merencanakan strategi selanjutnya.')
    
    st.markdown('\n*Data yang digunakan bersumber dari API dataset-dqlab dengan format csv. Silahkan untuk unduh data [disini](https://storage.googleapis.com/dqlab-dataset/retail_raw_reduced_data_quality.csv)*')
...
with row3_1:
    ...
    st.markdown('Deskripsi kolom pada tabel')
...
with row5_2:
    st.text('')
    st.markdown('Pada bulan {} {} di Provinsi {} diperoleh pendapatan total sebesar **{:,.2f}**.\
                 Total pendapatan terbesar terjadi di **{}** sebesar **{:,.2f}**\
                 dengan banyak transaksi yang terjadi sebanyak {:.0f} kali.'\
                    .format(" ".join(month), 
                            year, " - ".join(province),
    ...
    if(sum_revenue.shape[0] > 1):
        st.markdown('Sebaliknya total pendapatan terkecil terjadi di **{}** sebesar **{:,.2f}**\
                     dengan banyak transaksi yang terjadi sebanyak {:.0f} kali.'\
                         .format(sum_revenue['city'][sum_revenue.shape[0]-1],
                                 sum_revenue['sum_gmv_per_city'][sum_revenue.shape[0]-1],
                                 sum_revenue['count_trx_per_city'][sum_revenue.shape[0]-1]))
st.markdown('')

row5a_spacer1, row5a_1, row5a_spacer2 = st.columns((.2, 7.1, .2))
with row5a_1:
    st.markdown('**nb** : *Standar Total Revenue awal yang diinisialisasi adalah rata-rata dari total revenue tiap kota pada provinsi {}\
                 yakni sebesar {}. Anda dapat menaikkan atau menurunkan standar dengan menginputkan nilai pada tempat yang disediakan*'\
                 .format(" - ".join(province), stdrevenue))
...
with row11_1:
    st.header('Summary')
    st.markdown('Pada hasil analisa diperoleh beberapa fakta bahwa terdapat beberapa kota atau kabupaten pada provinsi\
                ...
                '.format(" - ".join(province), stdrevenue, citybelowstd, mapCityAndProduct))
    st.markdown('Pertumbuhan *new customer* yang paling baik terjadi di kota/kab **{}** sebesar **{:.2f}%** sehingga kota/kab tersebut bisa menjadi pacuan atau tolak ukur \
                 ...
                 cek laporan produk, buat promosi yang menarik dan sebagainya'
                .format(mean_of_perc['kota'][0], mean_of_perc['rataan_pertumbuhan'][0]))